...
def MNMF(data,k):
    ...
    for i in range(data.shape[0]):#计算二阶近似矩阵的值
        for j in range(data.shape[1]):
            S2[i,j]=np.dot(S1[i,:],S1[j,:])/(np.linalg.norm(S1[i,:])*np.linalg.norm(S1[j,:])+0.000000001)
    ...
    for i in range(N_MAX):#更新各个参数
        Mt=M
        M=M*(np.dot(S,U)/(np.dot(np.dot(M,U.transpose()),U)+0.000000001))#更新M矩阵
        Ut=U
        U=U*((np.dot(S.transpose(),M)+a*np.dot(H,C))/(np.dot(U,(np.dot(M.transpose(),M)+a*np.dot(C.transpose(),C)))+0.00000001))#更新U矩阵
        Ct=C
        C=C*(np.dot(H.transpose(),U)/(np.dot(np.dot(C,U.transpose()),U)+0.0000000001))#更新C矩阵
        ...
        Ht=H
        H=H*np.sqrt((np.sqrt(deta)-2*b*np.dot(B1,H))/(8*lamda*np.dot(np.dot(H,H.transpose()),H)+0.000000001))#更新H矩阵
